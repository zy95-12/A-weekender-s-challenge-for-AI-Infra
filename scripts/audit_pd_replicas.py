...
root=Path(__file__).resolve().parents[1]/'results/pd_replicas'
out=[]
...
for folder in sorted(root.iterdir()):
 ...
 for r in req:
  ...
  assert len({t['prefill_replica'] for t in steps})==1
  assert 0<=steps[0]['prefill_replica']<s['prefill_replicas']
  ...
  assert k['prefill_replica']==steps[0]['prefill_replica']
 ...
 s['prefill_requests_by_replica']={str(peer):sum(t['phase']=='prefill' and t['position_start']==0 and t['prefill_replica']==peer for t in rows) for peer in range(s['prefill_replicas'])}
...
keys=['variant','prefill_replicas','prefill_tp','prefill_window_per_replica','decode_window','chunk','duration_s','completed_qps','mean_ttft_ms','p99_ttft_ms','mean_tpot_ms','p99_tpot_ms','slo_attainment','start_cohort_slo_attainment','slo_pass','mean_first_front_wait_ms','mean_prefill_span_ms','mean_cloud_service_ms','prefill_chunks_per_s','all_requests_audited']
