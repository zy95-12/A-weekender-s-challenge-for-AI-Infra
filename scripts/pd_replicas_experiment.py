"""Two TP1 P replicas: bounded correctness and C24 comparison with TP2."""
...
def case(label,replicas,validation=False,seconds=90):
 ...
 try:
  ...
  with network_lock():
   ...
   if validation:
    run(folder/'validation.log',[PYTHON,'-m','scripts.pd_replicas_validate','--out',str(folder/'validation'),'--concurrency','24'])
    value=json.loads((folder/'validation/audit.json').read_text())
   else:
    run(folder/'point.log',[PYTHON,'-m','scripts.slo_point','--variant',label,'--concurrency','24','--seconds',str(seconds),'--cycles','4',
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--mode',choices=['validation','performance','confirm'],required=True);a=p.parse_args()
 ...
 (OUT/f'{a.mode}_source_hashes.json').write_text(json.dumps(hashes,indent=2))
 run(OUT/f'{a.mode}_demo_down.log',['./poc','down'],BASE)
 try:
  if a.mode=='validation':case('validation',2,True)
  else:
   seconds=180 if a.mode=='confirm' else 90
   values=[case(f'{a.mode}-tp2',1,seconds=seconds),case(f'{a.mode}-replicas',2,seconds=seconds)]
   (OUT/f'{a.mode}.json').write_text(json.dumps(values,indent=2))
 finally:
  run(OUT/f'{a.mode}_restore.log',['./poc','up','--wan','--ipc-mode','shm','--wire-fast','--tcp-buffer-mib','16'],BASE)
