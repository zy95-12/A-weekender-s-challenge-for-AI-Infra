...
async def main():
    ...
    parser.add_argument('--concurrency',type=int,choices=[1,16,24],default=16)
    ...
    async with httpx.AsyncClient(base_url='http://127.0.0.1:8000',timeout=180,trust_env=False) as client:
        ...
        for name,ids in [('current4k',prompt),('nonaligned257_a',other),('nonaligned257_b',other)]:
    ...
    for role,port in [('prefill',8001),('prefill_1',8003),('decode',8002)]:
        ...
        if role.startswith('prefill'):assert h['pd_reservations']==0,h
        (OUT/(role+'_health.json')).write_text(json.dumps(h,indent=2))
    live=Path((ROOT/'run/current_results').read_text())
    for name in ['split_trace.jsonl','pd_trace.jsonl','pd_kv_trace.jsonl','enterprise_config.json','cloud_prefill_config.json','cloud_prefill_1_config.json','cloud_decode_config.json','pd_kv_trace_1.jsonl']:
        shutil.copyfile(live/name,OUT/name)
    transfers=[json.loads(line) for p in OUT.glob('pd_kv_trace*.jsonl') for line in p.read_text().splitlines()]
    assert {r['prefill_replica'] for r in transfers}=={0,1}
    import numpy as np
    with np.load(OUT/'nonaligned257_a.npz') as a,np.load(OUT/'nonaligned257_b.npz') as b:
    ...
    result={'passed':True,'concurrency':args.concurrency,'requests':len(rows),'exact_kv_transfers':len(transfers),'exact_kv_chunks':chunk_count,'cancel_cases':2,
            'two_source_replicas':True,'cross_replica_teacher_force_exact':True,'nonaligned_prompt_tokens':257,'max_tokens_one':True}
