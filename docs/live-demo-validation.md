...
页面实测表展示从真实时间戳计算的企业、上行RPC、云侧、下行RPC及步外残差。RPC区间包含打包/IPC等，企业及云侧区间含CPU准备和调度，不能解释为纯GPU kernel时间或纯WAN传播时间。残差也不能全部归于单一调度器函数。原始请求、trace、配置与环境见 [baseline-c16-raw.zip](../demo/evidence/baseline-c16-raw.zip)，可运行 `python3 scripts/check_demo_evidence.py` 独立复算。

## Baseline / 优化系统开环对比

05 章节已切换到独立泊松到达的双系统对比，闭环吞吐扫描仅保留历史归档；demo 在 C16 实测耗时表下方展示通信、流水与调度分析结论。完整结果、各点时长及统计限制见 [开环实测报告](../demo/evidence/open-loop-report.md)，[CSV](../demo/evidence/open-loop-sweep.csv) 和 [原始请求、计划、trace及复现脚本](../demo/evidence/open-loop-raw.zip)。
...
本次共11点。优化系统最高已测合格点：目标到达率4.440/s、实际4.700/s、完成4.775 QPS；TTFT均值/P99 998.9/2307.0ms、TPOT 76.90/95.84ms，564条到达请求和573条完成请求均100%联合达标。更高目标4.744/s的点联合达标率89.42%，主要触及TPOT。Baseline三个点的到达联合达标率为100%、96.97%、52.83%；最低点仅18条到达请求，不能视为精确SLO容量上界。

8项开环/demo检查通过；11点原始指标、相同负载请求计划、输出文本、网络与排空检查通过，ZIP CRC及归档源码hash通过。浏览器核对全部11行数据、两系统均值/P99曲线、桌面/移动视图和CSV/PNG/ZIP/报告下载，未发现JavaScript错误；见 [浏览器验收](../demo/evidence/open-loop-browser-validation.json)。
