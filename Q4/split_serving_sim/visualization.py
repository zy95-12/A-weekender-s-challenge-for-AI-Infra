...
function selectBatch(batch){details.replaceChildren();const heading=document.createElement('strong');heading.textContent=`Batch B${batch.batch_id} — ${batch.resource} / ${batch.stage}`;details.append(heading);const summary=document.createElement('div');summary.textContent=`transaction=${batch.transaction_id??'-'} | execution=${batch.execution_mode||'pipelined'} | PP=${batch.pipeline_rank??0} | layers=${JSON.stringify(batch.layer_range)} | phase=${batch.phases.join('/')} | requests=${JSON.stringify(batch.request_ids)} | duration=${batch.duration_ms.toFixed(3)} ms | input=${batch.input_shape}`;details.append(summary);const membership=document.createElement('div');membership.textContent=`Prefill requests=${JSON.stringify(batch.prefill_request_ids||[])} | Decode requests=${JSON.stringify(batch.decode_request_ids||[])}`;details.append(membership);const relation=document.createElement('div');relation.textContent=`前驱 batches=${JSON.stringify([...(batchDependencies.get(batch.batch_id)||[])])} | 后继 batches=${JSON.stringify([...(batchDependents.get(batch.batch_id)||[])])}`;details.append(relation);if(batch.sub_operations_omitted){const notice=document.createElement('div');notice.textContent='该 batch 的逐算子明细因 simulation.max_detailed_trace_records 内存保护而省略；stage 总耗时仍保留。';details.append(notice);}const table=document.createElement('table');table.className='detail-table';const head=document.createElement('tr');for(const value of ['算子','子流','耗时 (ms)','Input shape','Cost source','修正系数']){const th=document.createElement('th');th.textContent=value;head.append(th);}table.append(head);for(const op of batch.sub_operations||[]){const tr=document.createElement('tr');for(const value of [op.name,streamKind(op),op.duration_ms.toFixed(3),op.input_shape,op.profile_source||'roofline',(op.profile_correction_factor??1).toFixed(3)]){const td=document.createElement('td');td.textContent=value;tr.append(td);}table.append(tr);}details.append(table);}
function render(){
