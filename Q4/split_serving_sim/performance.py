    ...
    def estimate(self, stage_name: str, items: Sequence[WorkItem]) -> PerformanceEstimate:
        ...
        for operator in operators:
            if operator.category == "communication":
                ...
                if not self.config.execution.host_submission.enabled:
                    overhead += launch
            ...
            coverage_key = f"{stage_name}/{phases}/tp{stage.tp_degree}"
            counts = self.coverage.setdefault(coverage_key, {})
            source = profile.source.split(":", 1)[0]
            counts[source] = counts.get(source, 0) + 1
            duration = profile.duration_s
            if (self.config.execution.host_submission.enabled
                    and profile.source == "roofline"
                    and operator.category != "communication"):
                # Uncalibrated Roofline included a serial launch constant.
                # GPU profile estimates already represent measured kernel costs.
                duration = max(0.0, duration - launch)
            if operator.category == "communication":
...
class NetworkModel:
    ...
    def estimate(self, stage: Stage, items: Sequence[WorkItem]) -> PerformanceEstimate:
        ...
        if stage in {Stage.PD_KV_START, Stage.PD_KV_COMMIT}:
            duration = self.config.scheduler.pd_disaggregation.control_latency_ms / 1000.0
            return PerformanceEstimate(0, 0, 0, 0, 0, 0, duration, duration, "PD control",
