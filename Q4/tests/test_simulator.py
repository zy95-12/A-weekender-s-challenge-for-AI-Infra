...
class SimulatorTest(unittest.TestCase):
    def test_ready_back_can_take_priority_over_older_front(self) -> None:
        for prefer_back in (False, True):
            with self.subTest(prefer_back=prefer_back):
                data = copied_toy_config()
                data['scheduler'] = {'prefer_ready_back': prefer_back}
                data['execution'] = {'preserve_batch_across_stages': False}
                sim = Simulator(parse_config(data))
                front = WorkItem(id=0, request_id=0, phase=Phase.DECODE,
                                 stage=Stage.EDGE_FRONT, token_start=32,
                                 token_count=1, context_tokens=32)
                back = WorkItem(id=1, request_id=1, phase=Phase.DECODE,
                                stage=Stage.EDGE_TAIL, token_start=32,
                                token_count=1, context_tokens=32, ready_time=1.0)
                resource = sim._resource_for_item(front)
                self.assertEqual(resource, sim._resource_for_item(back))
                sim.queues[resource] = [front, back]
                chosen = []
                class DispatchObserved(Exception):
                    pass
                def capture(resource, items):
                    chosen.extend(items)
                    raise DispatchObserved()
                sim._start_batch = capture
                with self.assertRaises(DispatchObserved):
                    sim._schedule_idle_resources()
                self.assertEqual(chosen, [back] if prefer_back else [front])
