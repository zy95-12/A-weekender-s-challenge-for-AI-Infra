...
- [摘要](#摘要)
- [01 背景](#01-背景)
- [02 大模型安全推理的业界洞察](#02-大模型安全推理的业界洞察)
- [03 Split-Infer 方案与安全性](#03-split-infer-方案与安全性)
- [04 功能实现](#04-功能实现)
- [05 真实系统优化](#05-真实系统优化)
- [06 性能优化空间与下一步优化方向](#06-性能优化空间与下一步优化方向)
- [07 推理性能建模](#07-推理性能建模)
- [08 页面交互与代码入口](#08-页面交互与代码入口)
...
以最高已测合格点为折算基准：测量窗口 T = 120 s，完成吞吐 Q₀ = 4.775 请求/s，窗口内完成 N = Q₀ × T = 573 条请求。延迟统计使用到达 cohort（564 条），此处容量折算使用完成请求数，两者口径不同。

Bᵢ 为节点 i 在窗口内计算命令区间取并集后的覆盖时间；Uᵢ 为覆盖率（使用 0～1 的小数）；Dᵢ 为按系统完成请求摊销的节点服务需求，单位 s/请求。命令时间包含 CPU / IPC / 拷贝，云侧已扣除命令排队，不是纯 GPU kernel 时间；命令区间之外也不能全部视为 GPU 空闲。
...
| 节点与工作 | 命令覆盖 Bᵢ (s) | 命令外时间 (s) | 覆盖率 Uᵢ | 服务需求 Dᵢ (ms/系统请求) | 折算系统容量 (QPS) |
|---|---|---|---|---|---|
| 企业 E：前层＋尾层，prefill / decode | 90.64 | 29.36 | 75.53% | 158.18 | 6.32 |
| 云 P0：中间层 prefill | 93.83 | 26.17 | 78.19% | 163.75 | 6.11 |
| 云 P1：中间层 prefill | 96.73 | 23.27 | 80.61% | 168.82 | 5.92 |
| 云 D：中间层 decode | 31.98 | 88.02 | 26.65% | 55.81 | 17.92 |

P0/P1 的服务需求按全部系统完成请求摊销，最后一列表示保留当前路由比例时的系统吞吐上限，不是单个 P 独立处理请求的速率。企业 E 的 90.64 s 中，prefill 前层 / 尾层为 32.97 / 23.71 s，decode 前层 / 尾层为 12.41 / 21.55 s。

当前路由下，瓶颈为 P1：Q系统,容量 ≈ min(6.32, 6.11, 5.92, 17.92) = 5.92 QPS。若两个 P 完全均衡，Q云P,均衡 ≈ 2Q₀ / (U_P0 + U_P1) = 6.01 QPS，仍低于企业 E 的 6.32 QPS；因此当前拓扑的局部容量估算约为 5.9～6.0 QPS。
...
ρᵢ* 是满足联合 SLO 时节点允许的覆盖率，需要用开环实测或经过校准的仿真确定，不能直接设为 100%。4.775 QPS 约为上述满载容量估算的 80%；实际完成 5.042 QPS 时，TPOT P99 已达 106.30 ms，说明 SLO 在命令服务打满前就已触顶。

折算假设 batch 效率、路由比例和每请求服务成本不变，并近似忽略有限窗口内跨边界请求的影响。它用于判断瓶颈和优化方向，不是独立验证的容量预测；增加负载或让 D 承接 prefill 后，需要重新测量服务需求。
...
Demo 第七部分提供开环到达率输入和硬件/模型下拉项，每个点实际执行仿真、不缓存结果：
