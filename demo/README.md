...
- **精度**：只展示 Split 4/27/5 对原生完整 TP2/full-prefill 的 8 道 GSM8K 真实归档结果。逐绝对位置对齐，decode 使用同一 teacher-forcing 序列；不比较自由回答。`accuracy-samples.json` 展示这 8 组不同输入。原始 logits、模型版本和门槛见 [精度报告](../docs/gsm8k-validation.md)。
